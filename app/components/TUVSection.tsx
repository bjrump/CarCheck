...
import { formatDate, getMaintenanceStatus, getStatusColorClass, getStatusText, calculateTimeProgress, formatTimeElapsed } from '@/app/lib/utils';
...
export default function TUVSection({ car, onUpdate }: TUVSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/cars/${car.id}/tuv`, {
        method: 'PUT',
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => setFormData({ ...formData, lastAppointmentDate: e.target.value || null })}
          ...
          <div className="rounded-xl bg-accent/10 p-3 text-sm text-accent">
            <strong>Hinweis:</strong> Der nächste TÜV-Termin wird automatisch auf 2 Jahre nach dem letzten Termin berechnet.
          </div>
    ...
    <div className="glass rounded-2xl p-6">
      ...
      <div className="space-y-2">
        <div className="flex justify-between">
          ...
          <span className="font-medium">{formatDate(car.tuv.lastAppointmentDate)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Nächster Termin:</span>
          <span className="font-medium">{formatDate(car.tuv.nextAppointmentDate)}</span>
        </div>
