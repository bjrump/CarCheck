...
import { calculateNextInspectionDateByYear, calculateNextInspectionDateByKm, getEarliestDate } from '@/app/lib/utils';
...
export async function PUT(
  ...
  try {
    ...
    let lastInspectionDate = body.lastInspectionDate !== undefined ? body.lastInspectionDate : car.inspection.lastInspectionDate;
    ...
    const intervalYears = typeof body.intervalYears === 'number' && !isNaN(body.intervalYears) && body.intervalYears > 0
      ? body.intervalYears
      : car.inspection.intervalYears;

    const intervalKm = typeof body.intervalKm === 'number' && !isNaN(body.intervalKm) && body.intervalKm > 0
      ? body.intervalKm
      : car.inspection.intervalKm;

    const lastInspectionMileage = body.lastInspectionMileage !== undefined
      ? (body.lastInspectionMileage === null || body.lastInspectionMileage === ''
          ? null
          : (typeof body.lastInspectionMileage === 'number'
              ? body.lastInspectionMileage
              : (typeof body.lastInspectionMileage === 'string' && body.lastInspectionMileage.trim() !== ''
    ...
    const updatedInspection: Inspection = {
      ...car.inspection,
      ...body,
      lastInspectionDate: lastInspectionDate !== undefined ? lastInspectionDate : car.inspection.lastInspectionDate,
      intervalYears,
      intervalKm,
      lastInspectionMileage: lastInspectionMileage !== undefined ? lastInspectionMileage : car.inspection.lastInspectionMileage,
    };

    // Calculate next inspection dates based on intervals
    if (updatedInspection.lastInspectionDate) {
      updatedInspection.nextInspectionDateByYear = calculateNextInspectionDateByYear(
    ...
    if (updatedInspection.lastInspectionDate && updatedInspection.lastInspectionMileage !== null) {
      updatedInspection.nextInspectionDateByKm = calculateNextInspectionDateByKm(
        updatedInspection.lastInspectionDate,
    ...
    const updatedCar = await updateCar(resolvedParams.id, { inspection: updatedInspection });

    return NextResponse.json(updatedCar);
  } catch (error) {
    console.error('Fehler beim Aktualisieren der Inspektions-Informationen:', error);
    return NextResponse.json(
      {
