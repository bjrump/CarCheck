import { format, parseISO, addYears, differenceInDays, differenceInMonths } from 'date-fns';
...
export function calculateNextInspectionDateByYear(
  ...
  try {
    ...
    const nextDate = addYears(lastDateObj, intervalYears);
    return nextDate.toISOString();
  } catch (error) {
...
// Calculate next inspection date by KM (estimate based on average km/month)
export function calculateNextInspectionDateByKm(
  ...
  try {
    ...
    // If already exceeded the interval, calculate from current date/mileage
    if (kmUntilNextInspection <= 0) {
      // Already exceeded - estimate based on when next interval will be reached
      // Calculate from current date forward
      const nextMileageTarget = lastMileage + intervalKm;
      const kmNeeded = nextMileageTarget - currentMileage;

      // Estimate date based on average km per month (assuming 1000 km/month)
      const estimatedMonths = Math.max(0, kmNeeded / 1000);
      const today = new Date();
      const nextDate = new Date(today);
      nextDate.setMonth(nextDate.getMonth() + Math.ceil(estimatedMonths));

      return nextDate.toISOString();
    }

    // Estimate date based on average km per month (assuming 1000 km/month)
    const estimatedMonths = kmUntilNextInspection / 1000;
...
export function getEarliestDate(date1: string | null, date2: string | null): string | null {
...
export function formatTimeElapsed(
  ...
  return `${elapsed.months} Monat${elapsed.months !== 1 ? 'e' : ''}, ${elapsed.days} Tag${elapsed.days !== 1 ? 'e' : ''}`;
}
